...
// 客户端统一上传模块：优先使用预签名URL直接上传到R2以绕过Vercel的4.5MB大小限制；如果失败则降级使用服务端转发接口。
const uploadFile = async (file: File, folder: string, accessToken: string): Promise<string> => {
  try {
    // 1. 尝试获取预签名上传 URL
    ...
    if (presignRes.ok) {
      ...
      if (uploadUrl && publicUrl) {
        console.log(`[Upload] Using presigned URL to upload ${file.name} directly to R2...`);
        // 2. 使用 PUT 直接上传至 Cloudflare R2
        ...
        if (uploadRes.ok) {
          console.log(`[Upload] Direct upload succeeded: ${publicUrl}`);
          return publicUrl;
        } else {
          console.warn('[Upload] Direct upload failed, status:', uploadRes.status, 'falling back to server upload...');
        }
      }
    } else {
      console.warn('[Upload] Failed to get presigned URL, status:', presignRes.status, 'falling back to server upload...');
    }
  } catch (err) {
    console.error('[Upload] Error in presigned upload flow, falling back to server upload:', err);
  }

  // 3. 降级使用 /api/upload/r2 服务端中转接口（受Vercel 4.5MB Payload限制限制）
  console.log(`[Upload] Falling back to server upload for ${file.name}...`);
      ...
      try {
        ...
        if (text) {
          if (text.includes('Request Entity Too Large') || res.status === 413) {
            errorMsg = '文件大小超限(Vercel限制最大4.5MB)。请确保上传的是音频压缩包，或联系管理员开启直传通道。';
          } else {
...
function ProfileContent() {
    ...
    const renderPlaylistDesign = (baseImage?: HTMLImageElement) => {
      ...
      canvas.toBlob(async (blob) => {
        ...
        try {
          ...
          const coverUrl = await uploadFile(fileToUpload, 'covers', session.access_token);
          setNewPlaylistCoverUrl(coverUrl);
        } catch (e: any) {
          console.error(e);
  ...
  const handleCreatePlaylist = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (newPlaylistCoverSource === 'upload' && newPlaylistCoverFile) {
        ...
        coverUrl = await uploadFile(compressedCover, 'covers', session.access_token);
      }
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const audioUrl = await uploadFile(audioFile, 'songs', session.access_token);

      console.log('正在对封面进行智能图形学算法优化...');
      const compressedCover = await compressImage(coverFile);
      const coverUrl = await uploadFile(compressedCover, 'covers', session.access_token);
  ...
  const handleEditSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editCoverSource !== 'keep' && editCoverFile) {
        ...
        finalCoverUrl = await uploadFile(compressedCover, 'covers', editSession.access_token);
      }
  ...
  const handleAvatarFileUpload = async (file: File) => {
    ...
    try {
      ...
      const url = await uploadFile(file, 'avatars', session.access_token);
      setEditAvatar(url);
    } catch (err: any) {
